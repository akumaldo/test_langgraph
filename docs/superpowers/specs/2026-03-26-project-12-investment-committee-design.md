...
These expansions are planned but not part of the initial build. The architecture is designed to support them without changes to the core deep-dive workflow.

### Phase 2 Expansion — Screening & Comparison

**Goal:** "Find me undervalued mid-caps in healthcare" — screen by financial criteria, compare candidates, rank with reasoning.

**Design:**
- New CrewAI crew that uses FMP screener tools (`search-company-screener` with financial filters: sector, market cap range, P/E range, margin thresholds, etc.)
...
- `compare {TICKER1} {TICKER2} {TICKER3}` — head-to-head comparison

### Phase 3 Expansion — Monitoring & Alerts
...
- Monitoring check (manual trigger or scheduled) that scans each watchlist ticker for:
  - Earnings surprises (actual vs estimate from `earnings-company`)
...
- `briefing` — run on-demand scan
- `watchlist` — show current watchlist

### Why This Architecture Supports the Roadmap

- **Screening** reuses `phases/gathering.py` and `phases/valuation.py` — same code, different orchestration
- **Monitoring** adds a new LangGraph graph (not nested inside the deep-dive) that calls the same FMP tools
